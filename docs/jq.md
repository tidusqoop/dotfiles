...
> JSON 데이터를 파싱, 필터링, 변환하는 CLI 도구. API 응답, K8s 출력, Terraform state 처리에 필수.
...
```bash
# select: 조건에 맞는 항목만
echo '[{"name":"kim","age":30},{"name":"lee","age":25}]' | jq '.[] | select(.age > 27)'
# → {"name":"kim","age":30}

# 문자열 포함 여부
echo '[{"name":"backend"},{"name":"frontend"}]' | jq '.[] | select(.name | contains("back"))'
...
# 배열에서 특정 필드만 추출
echo '[{"name":"kim","age":30},{"name":"lee","age":25}]' | jq '[.[] | .name]'
# → ["kim","lee"]
...
# 특정 라벨의 Pod
kubectl get pods -o json | jq -r '.items[] | select(.metadata.labels.app == "nginx") | .metadata.name'
```
...
# 정렬
curl -s https://api.github.com/users/octocat/repos | jq 'sort_by(.stargazers_count) | reverse | .[:5] | .[].name'
```
...
| `-e` | 결과가 false/null이면 exit code 1 반환 |
| `-s` | 여러 JSON 입력을 하나의 배열로 합침 (slurp) |
...
```bash
NAME="kim"
echo '{"name":"kim","age":30}' | jq --arg n "$NAME" 'select(.name == $n)'
```
