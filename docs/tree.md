...
> 디렉토리 구조를 트리 형태로 시각화하는 도구. 프로젝트 구조 파악, 문서화, 디버깅에 유용하다.
...
| `-a` | 숨김 파일/디렉토리 포함 |
| `-f` | 각 파일의 전체 경로 표시 |
| `-I <패턴>` | 특정 패턴 제외 (예: `-I "node_modules"`) |
| `-P <패턴>` | 특정 패턴만 표시 (예: `-P "*.go"`) |
| `--prune` | 빈 디렉토리 숨김 (`-P`와 함께 사용) |
| `-s` | 파일 크기 표시 |
...
| `-C` | 색상 출력 (기본 활성화) |
| `--noreport` | 하단 요약 (directories, files) 숨김 |
